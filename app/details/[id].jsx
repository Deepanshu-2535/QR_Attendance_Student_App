...
const Id = () => {
    ...
    const percentage = Math.round((attendanceDetails.summary.attended / attendanceDetails.summary.total)*100);
    const fillcolor = percentage>75?colors.success: percentage<25?colors.danger:colors.primary;
    return (
        <>
            <Stack.Screen options={{title:"",headerShadowVisible:false,headerStyle:{backgroundColor:colors.background,elevation:0}}}/>
            <View style={styles.container}>
                <View className="flex-row">
                    <View className="w-[65%] pr-3">
                        <Text className="font-bold text-4xl mb-1 text-text">{attendanceDetails.subjectName}</Text>
                ...
                <View className="flex-row">
                    <View className="flex-row items-center justify-between flex-1">
                        ...
                        <Text className="text-success font-semibold text-lg">{attendanceDetails.summary.attended}</Text>
                    </View>
                    <Divider orientation="vertical" margin={10}/>
                    <View className="flex-1 flex-row items-center justify-between">
                        <Text className="text-lg font-semibold">Missed</Text>
                        <Text className="text-danger font-semibold text-lg">{attendanceDetails.summary.total - attendanceDetails.summary.attended}</Text>
                    </View>
                </View>
            </View>
